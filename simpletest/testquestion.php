...
require_once($CFG->dirroot . '/question/type/transcription/question.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_transcription_question_test extends UnitTestCase {
    public function test_compare_string_with_wildcard() {
        // Test case sensitive literal matches.
        $this->assertTrue(qtype_transcription_question::compare_string_with_wildcard(
                'Frog', 'Frog', false));
        $this->assertFalse(qtype_transcription_question::compare_string_with_wildcard(
                'Frog', 'frog', false));
        $this->assertTrue(qtype_transcription_question::compare_string_with_wildcard(
                '   Frog   ', 'Frog', false));
        $this->assertFalse(qtype_transcription_question::compare_string_with_wildcard(
    ...
    public function test_is_complete_response() {
        $question = test_question_maker::make_question('transcription');
    ...
    public function test_get_correct_response_escapedwildcards() {
        $question = test_question_maker::make_question('transcription', 'escapedwildcards');

        $this->assertEqual(array('answer' => 'x*y'), $question->get_correct_response());
    }

    public function test_get_question_summary() {
        $sa = test_question_maker::make_question('transcription');
    ...
    public function test_classify_response_no_star() {
        $sa = test_question_maker::make_question('transcription', 'frogonly');
        $sa->start_attempt(new question_attempt_step(), 1);
